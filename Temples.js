...
(function define(context, $) {
	...
	Renderer.parse = function($root) {
		...
		var bindings  = this.bindings = [],
			notContainedInIterator = function() {return ($(this).parentsUntil($root, "[data-iterate]").length == 0);};
		...
		$("*[data-iterate]", $root)
			...
			.each(function(i, elt) {
				var $elt = $(elt),
					iterateExpr = $elt.attr("data-iterate"),
	...
	function Iterator($elt, expr, template) {
		...
		if (exprParts.length == 1) { // simply <collection> was passed
			this.seed = exprParts[0];
			var seedLetters = exprParts[0].split("");
			if (seedLetters.pop() != "s") {
			...
			this.variableName = seedLetters.join(""); // Get rid of last letter 's'
		} else {
