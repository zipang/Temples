...
var splitPath = (path) => path.split(/[,[\].]+?/).filter(Boolean),
	getProperty = (source, path, defaultValue) => {
	...
	isArrayIndex = (key) => /^(0|[1-9][0-9]*)$/.test(key),
...
var prepare = (source, { removeDataBindings = true, templesComponents = [] } = {}) => {
	...
	return async (data) => {
		...
		try {
			const { Renderer: Renderer2 } = await Promise.resolve().then(
				() => (init_engine(), exports_engine)
			);
			let componentClass;
			if (templesComponents.length > 0) {
				({ TemplesComponent: componentClass } = await Promise.resolve().then(
					() => (init_component(), exports_component)
				));
...
var componentStyles = (components) =>
	components
